# README — Données de mobilisation des fonds européens

> Chaîne de traitement des financements européens perçus par les acteurs
> français, en gestion **directe** (FTS) et en gestion **partagée** (Kohesio).
>
> | Notebook | Rôle | Sections |
> |---|---|---|
> | `FTS_France_PREPARATION_ENRICHISSEMENT` (v5_41) | identification et qualification des bénéficiaires FTS | §1 à §17 |
> | `KOHESIO_beneficiaires_COMPLET_v6` | siretisation des bénéficiaires Kohesio | §18 |
> | `REPARTITION_CANAUX` | croisement des deux sources et répartition par canal | §19 à §21 |
>
> Les nouveautés du pipeline FTS depuis la version précédemment documentée
> (v5_19) sont récapitulées en [section 17](#17-nouveautés-depuis-la-v5_19).
...
**Chaîne Kohesio et croisement**

18. [Siretisation des bénéficiaires Kohesio](#18-siretisation-des-bénéficiaires-kohesio)
19. [**Croisement FTS × Kohesio**](#19-croisement-fts--kohesio)
20. [Restitution dans MicroStrategy](#20-restitution-dans-microstrategy)
21. [Limites connues de la chaîne Kohesio](#21-limites-connues-de-la-chaîne-kohesio)
...
| **Canal** | mode de gestion du financement : directe (FTS) ou partagée (Kohesio) |
| **Format long / large** | long = une ligne par entité × année × canal (graphiques) ; large = montants en colonnes (tableaux) |
| **Montant programmé** | montant Kohesio couvrant toute la durée du projet, par opposition à un engagement annuel |
...
| [Kohesio](https://kohesio.ec.europa.eu/) | projets de la politique de cohésion (gestion partagée) |
| [Règlement (UE) 2021/1060](https://eur-lex.europa.eu/eli/reg/2021/1060/oj?locale=fr) | base légale de la publication Kohesio (art. 49 §3) |
...
## 18. Siretisation des bénéficiaires Kohesio

> Notebook `KOHESIO_beneficiaires_COMPLET_v6.ipynb`.
> Ce notebook **ne réécrit aucun moteur** : il contient le moteur v6 du pipeline
> FTS, intact, suivi de cellules d'adaptation qui lui présentent les données
> Kohesio au format attendu.

### 18.1 Pourquoi un second notebook

**Kohesio** recense les projets de la politique de cohésion — fonds en gestion
**partagée**, hors périmètre du FTS. Le fichier publié ne comporte **ni SIREN,
ni SIRET, ni numéro de TVA** : sans identification, aucun rapprochement n'est
possible avec les référentiels français ni avec le FTS.

L'objectif est donc le même que pour le FTS — retrouver le SIREN de chaque
bénéficiaire — mais la source est différente.

### 18.2 Fichiers d'entrée

Deux exports Kohesio circulent, à ne pas confondre :

| Fichier | Contenu | Colonne du nom |
|---|---|---|
| `latest_FR-pp21-27-latest.xlsx` | **opérations** 2021-2027 (5 759 lignes, 43 colonnes) | `Beneficiary_Name` |
| `latest_FR-latest.xlsx` | **référentiel des bénéficiaires** (19 585 lignes, 4 colonnes) | `BeneficiaryLabel` |

Le référentiel des bénéficiaires est plus large que le fichier d'opérations
21-27 : il couvre également les périodes antérieures. Les 3 556 bénéficiaires du
21-27 y sont tous présents, accompagnés de 16 029 autres entités.

> Le fichier `latest_FR-latest.xlsx` ne contient **ni adresse, ni code postal**.
> La recherche s'effectue sur le seul nom, ce qui est le cas le moins favorable
> pour lever les homonymes : le taux d'échec y est plus élevé que sur le FTS.

### 18.3 Adaptation au moteur figé

Le moteur détecte ses colonnes par mots-clés (§2.1). Les cellules d'adaptation
construisent donc une vue de travail portant les intitulés attendus :

| Colonne attendue par `detecter_colonnes` | Source Kohesio |
|---|---|
| `Name of beneficiary` | `Beneficiary_Name` ou `BeneficiaryLabel` |
| `VAT number of beneficiary` | **créée vide** (voir ci-dessous) |
| `Beneficiary country` | `Country`, ou « France » par défaut |
| `Postal code` | `Postal_Code` si présent |

> **Piège à connaître.** La colonne de TVA doit être une **chaîne vide `""`**,
> et non `pd.NA`. La fonction `_tva_manquante` du moteur considère les valeurs
> nulles de pandas comme une TVA *présente* : avec `NA`, le moteur annonce
> « 0 TVA manquant » et **ne cherche rien**. Le fichier doit par ailleurs être
> lu avec `dtype=str`, comme le FTS.

Un garde-fou en tête de notebook vérifie que le moteur est bien chargé et
signale explicitement les fonctions manquantes plutôt que d'échouer plus loin
sur une erreur obscure.

### 18.4 Colonne `Annee`

Kohesio ne comporte **aucune colonne d'année**. Le notebook en dérive une, à
partir de la date de début de financement, insérée **juste avant**
`Operation_Start_Date` :

```
… Country | Annee | Operation_Start_Date | Operation_End_Date …
```

> C'est une **convention** : le montant entier du projet est rattaché à son
> année de démarrage, alors qu'il s'exécute sur plusieurs exercices. Elle situe
> une dynamique de programmation, elle ne mesure pas une consommation annuelle.

### 18.5 Sortie

| Fichier | Contenu |
|---|---|
| `KOHESIO_beneficiaires_siretise.xlsx` | colonnes d'origine **intactes** + `Annee` + `SIREN`, coloriées en bleu |
| `KOHESIO_beneficiaires_RAPPORT_TVA.xlsx` | rapport d'audit, colonnes identiques au rapport FTS (§2.2) |

La sortie ne conserve **que le SIREN** parmi les colonnes produites par le
moteur. Celui-ci continue de calculer SIRET, TVA, forme juridique et NAF — il
est figé — mais ces colonnes ne sont pas recollées. Pour en récupérer une,
l'ajouter à la liste `_CANDIDATES` de la cellule de recollage.

> Le temps de traitement reste identique : l'API renvoie tous ces champs dans le
> même appel, restreindre la sortie n'accélère rien.

---

## 19. Croisement FTS × Kohesio

> Notebook `REPARTITION_CANAUX.ipynb`.
> Produit un fichier unique montrant, pour chaque bénéficiaire et chaque année,
> ce qu'il capte en gestion **directe** (FTS) et en gestion **partagée**
> (Kohesio).

### 19.1 Entrées

Les deux fichiers **déjà siretisés** : la sortie France du pipeline FTS (§2.2)
et la sortie du notebook Kohesio (§18.5).

### 19.2 La clé est le SIREN, jamais le nom

Le nom ne peut pas servir de clé : le CNRS apparaît sous **9 libellés** dans
Kohesio (délégations régionales) et sous son nom légal dans le FTS. Le SIRET ne
convient pas davantage — il désigne l'établissement, quand Kohesio donne souvent
celui d'une délégation et le FTS celui du siège.

Le SIREN identifie la **personne morale** : c'est le seul niveau où les deux
bases se parlent, et la consolidation des délégations sous une entité unique est
recherchée.

> **Le SIREN se manipule en texte, zéros de tête compris.** Lu comme un nombre,
> `012345678` devient `12345678` : la jointure échoue **silencieusement**, sans
> aucune erreur, et les entités concernées ne se rapprochent jamais.

### 19.3 Nom de référence

Le nom unique d'une entité est le **`Nom API`** — l'écriture officielle issue de
l'annuaire, produite par le même moteur dans les deux bases, donc identique de
part et d'autre.

| Fichier | Intitulé exact |
|---|---|
| FTS | `Nom API` (avec **espace**) |
| Kohesio | `Nom_API` (avec **underscore**) |

Retenir le nom le plus fréquent serait une erreur : sur le CNRS, la fréquence
désignerait une délégation régionale plutôt que l'entité nationale. La colonne
`SOURCE_NOM_REFERENCE` trace l'origine du nom retenu (SIRENE, nom le plus
fréquent, valeur unique), ce qui distingue les noms certifiés des conventions.

Les colonnes `NOM_FTS` et `NOM_KOHESIO` conservent l'écriture propre à chaque
base.

### 19.4 Assiettes de montants

| Canal | Colonne | Ce qu'elle mesure |
|---|---|---|
| Gestion directe | `Beneficiary's contracted amount (EUR)` | montant contractualisé par l'UE |
| Gestion partagée | `Project_EU_Budget` | **part européenne** du projet |

Ces deux assiettes sont **homogènes** : l'une comme l'autre mesure une
contribution de l'Union. C'est le choix pertinent pour répondre à « que capte
cette entité de l'Europe, et par quel canal ».

Alternative documentée en tête de notebook :
`Total_Eligible_Expenditure_amount` mesure la **dépense éligible totale**,
cofinancement national compris. L'écart est important — 511 821 € de dépense
éligible pour 307 093 € de part UE sur un même projet — et déplace sensiblement
la répartition.

> **Colonnes interdites.** Ne jamais sommer une colonne déjà agrégée par
> bénéficiaire, comme `Montant_total_beneficiaire` : sa valeur est répétée sur
> chaque ligne et sa somme multiplie le montant réel. Le notebook refuse
> explicitement ces colonnes.

### 19.5 Les deux canaux ne sont pas additionnables

Le montant FTS est un **engagement contractualisé sur un exercice**. Le montant
Kohesio est un montant **programmé sur toute la durée du projet**, rattaché à
son année de démarrage (§18.4). Leur somme n'a pas de signification comptable.

La colonne produite se nomme donc `MONTANT_TOTAL_INDICATIF` : elle sert à
**hiérarchiser** les acteurs — qui capte le plus, quel classement — jamais à
produire un montant officiel ni à répondre à « combien la France a-t-elle
capté ».

### 19.6 Conversion numérique

Les deux fichiers peuvent utiliser des formats différents : anglo-saxon
(`1,234.56`) côté export européen, français (`1 234,56`) côté national. Une
conversion naïve produit des `NaN` comptés comme zéro, ou des montants
multipliés par mille.

Règle appliquée, sans ambiguïté :

- virgule **et** point → le **dernier des deux** est le séparateur décimal
- virgule **seule** → séparateur **décimal**, sauf motif anglo-saxon strict
  (`^\d{1,3}(,\d{3})+$`)
- point seul → séparateur décimal

> L'erreur à ne pas commettre : traiter « virgule suivie de 3 chiffres » comme
> un séparateur de milliers. En format français, `1234,567` vaut **1 234,57 €**
> et non 1 234 567 € — c'est ainsi qu'une commune se retrouve avec des centaines
> de millions.

Un **contrôle de vraisemblance** signale toute entité dépassant 100 M€ sur une
année et affiche systématiquement le maximum observé ainsi que le montant total
converti par canal.

### 19.7 Sorties

`REPARTITION_CANAUX_MICROSTRATEGY.xlsx`, deux feuilles complémentaires :

| Feuille | Grain | Usage |
|---|---|---|
| `POUR_TABLEAU_large` | 1 ligne par SIREN × année, montants en colonnes | tableaux, tris, métriques |
| `POUR_GRAPHIQUE_long` | 1 ligne par SIREN × année × **CANAL** | **graphiques empilés** |

> Un diagramme à barres empilées exige le format **long** : on ne peut pas
> empiler deux métriques distinctes, il faut une dimension de découpage. C'est
> la cause d'échec la plus fréquente au moment de construire le visuel.

Colonnes principales :

```
SIREN | NOM_API | SOURCE_NOM_REFERENCE | ANNEE
MONTANT_FTS | MONTANT_KOHESIO | MONTANT_TOTAL_INDICATIF
PART_DIRECTE_PERIODE | PART_PARTAGEE_PERIODE | PRESENCE_PERIODE
```

`PRESENCE_PERIODE` vaut « FTS seul », « Kohesio seul » ou « Les deux » : elle
permet d'isoler les entités multicanal sans filtrage en amont.

### 19.8 Interpréter un déséquilibre entre canaux

Si un canal paraît écrasé, vérifier dans cet ordre :

1. **le format numérique** (§19.6) — cause la plus fréquente ;
2. **le périmètre temporel** — un FTS 2014-2025 face à un Kohesio 2021-2027
   sous-représente mécaniquement la gestion partagée ;
3. **le taux de siretisation** — les lignes sans SIREN sont exclues du
   croisement, et Kohesio se siretise moins bien faute d'adresse (§18.2) ;
4. **l'effet de calendrier** — préférer la répartition sur la période complète,
   nettement plus robuste que l'annuelle ;
5. **un biais réel de composition** — les opérateurs de recherche dominent la
   gestion directe, les collectivités et entreprises la gestion partagée.

---

## 20. Restitution dans MicroStrategy

### 20.1 Deux situations selon les droits

| Accès | Approche |
|---|---|
| Entrepôt + Architect | modèle en étoile, attributs et faits, métriques conditionnelles |
| Data visualisation seule | **un fichier unique dénormalisé** importé, aucune liaison à créer |

Sans accès à l'entrepôt, tout le paramétrage d'attributs et de faits est
inapplicable : l'import d'un fichier prêt à l'emploi est alors la seule voie —
et elle fonctionne dans les deux situations.

### 20.2 Diagramme à barres empilées

À partir de la feuille `POUR_GRAPHIQUE_long` :

| Zone | Objet |
|---|---|
| Catégories (axe) | `NOM_API`, puis `ANNEE` |
| Couleur / découpage | **`CANAL`** |
| Mesure | `Sum(MONTANT)` |
| Sous-type | *Empilé* (et non *Groupé*) |

Pour lire la répartition plutôt que les montants, basculer sur **Empilé à
100 %** : plus parlant pour comparer des entités de tailles très différentes.

### 20.3 Pièges fréquents

| Symptôme | Cause probable | Solution |
|---|---|---|
| La somme des deux montants retourne `0` | les métriques ne se rencontrent jamais sur la même ligne | afficher les deux métriques séparément ; vérifier le type du SIREN dans chaque dataset |
| Zéros de tête du SIREN perdus | colonne importée en numérique | forcer le type **Texte** à l'écran d'aperçu d'import |
| `ANNEE` traitée comme une mesure | MicroStrategy classe les colonnes numériques en métriques | la déclarer comme **attribut** à l'import |
| `ZeroIfNull` non reconnu | orthographe (`I` majuscule, souvent confondu avec `l`), ou fonction absente de la version | insérer depuis la liste des fonctions ; sinon `IfNull(x, 0)` ou un `Case` |
| Un bénéficiaire mono-canal disparaît | jointure interne entre métriques | régler `Metric Join Type` = **Outer** (VLDB) |
| Montants gonflés | jointure plusieurs-à-plusieurs entre les deux sources | ne pas joindre : **empiler**, ou pré-agréger au grain SIREN × année |

> **Contrôle avant diffusion.** Comparer le montant d'une entité affiché dans
> MicroStrategy avec celui du fichier produit par le notebook. Un écart signale
> une multiplication de lignes ou un problème de clé : le fichier Python sert de
> **référence de contrôle** du modèle BI.

---

## 21. Limites connues de la chaîne Kohesio

- **Géolocalisation inexploitable.** Le géocodage de Kohesio procède par nom de
  commune sans désambiguïsation des homonymes et se trompe sur environ **12 %**
  des cas : Pradelles (Haute-Loire) classée dans le Nord, communes de La Réunion
  situées en métropole. `LAU_IDs` **et** `NUTS3_Code` dérivant du même
  géocodage, ils sont cohérents entre eux **même quand ils sont faux** :
  l'erreur n'est pas détectable en interne. `NUTS1` et `NUTS2` sont entièrement
  vides.
- **Montants programmés, non exécutés.** `Project_EU_Budget` est un montant
  programmé : ni engagé au sens comptable, ni payé. Kohesio ne permet donc pas
  de calculer un taux de consommation.
- **Périmètre partiel.** L'export Kohesio ne couvre que le FEDER, le FSE+ et le
  FTJ. Le FEAMPA, le FAMI, le FSI et l'IGFV en sont absents.
- **Fraîcheur lente.** Deux extractions successives du fichier `latest` se sont
  révélées strictement identiques à six semaines d'intervalle : la fréquence
  réelle de mise à jour se mesure, elle ne se suppose pas.
- **Taux de siretisation à vérifier avant toute interprétation.** Une entité
  siretisée d'un seul côté apparaît à tort comme présente sur un seul canal.
